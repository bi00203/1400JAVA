...
// 멤버 메서드는 최초 객체 생성시 정의 부분이 클래스 영역(공유 메모리에)에 저장된다
// 클래스 영역에 저장 : static 변수, static 메서드, 인스턴스 메서드( 1회이상호출), 생성자 메서드
// 일반 멤버 메서드는 객체 1회이상 생성시 클래스 영역에 저장 된다(재사용성을 위함)
...
// Downcasting
//------------------------
// 하위 클래스의 참조변수 = 상위 클래스 참조변수(상위 클래스 객체)
// 일반적으로 업캐스팅 된 상태에서 확장된 멤버에 접근하기 위해서
// 해당 자료형으로 강제 형변환하는 작업
...
//		a3.Move();		 // 업캐스팅된 상태에서는 하위객체의 확장된 멤버에 접근이 불가능
		...
		Dog down = (Dog)a3;	 // 컴파일 에러! 자료형 불일치, 문제소지가 있다..
						     // 강제 형변환 해야함 -> Downcasting
		
		down.name="뽀삐";
		down.Move();
		
		// 상위클래스 instanceof 하위클래스 = 상속 관계 여부를 true/false로 확인
